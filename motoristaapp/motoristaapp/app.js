...
function loginSuccess() {
    state.user = { name: 'João Driver', email: 'demo@GiroPro.com', plan: 'pro' };
    showPage('app-page');
...
function generatePDF() {
    ...
    // 2. Visual Feedback
    showToast("Gerando Relatório de Alta Performance... 📄", 5000);

    const options = {
        margin: [15, 10, 15, 10],
        filename: `giropro-performance-report-${new Date().toISOString().split('T')[0]}.pdf`,
    ...
    const chartsContainer = element.querySelector('.charts-full-grid');
    ...
    if (chartsContainer) {
        ...
        chartsContainer.style.gap = '40px';
    }
